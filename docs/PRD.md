...
#### Feature 32 — Sub-Jenis "Izin Tidak Full": Telat, Izin di Tengah Jam Kerja, Pulang Cepat ✅
**Deskripsi:** Setelah memilih jenis **"Izin Tidak Full (Telat, Pulang Cepat, Pertengahan)"** (enum tetap `SETENGAH_HARI`, dulu berlabel "Izin Setengah Hari" — lihat Feature 26) di dropdown "Jenis Pengajuan", muncul pilihan sub-jenis tambahan — pola yang sama seperti pilihan tipe surat dokter di Izin Sakit. Tiga sub-jenis, tiap satu mewajibkan jam yang relevan (format `HH:mm`, `<input type="time">`):
...
Aturan lain untuk Izin Tidak Full **tidak berubah** dari eks-Izin Setengah Hari: tetap 1 tanggal (`JENIS_SATU_TANGGAL`), tetap dikecualikan penuh dari persentase kehadiran (`JENIS_TIDAK_HITUNG_KEHADIRAN`), tetap kena aturan cegah-duplikasi per tanggal (Feature 30) — duplikasi dicek per `jenis`, belum per sub-jenis (jadi tidak bisa ajukan 2x di tanggal sama walau sub-jenisnya beda).

**Susulan — batas waktu pengajuan Telat, kecuali admin (16 Agu 2026):** untuk mencegah karyawan beralasan sakit/lain-lain lalu belakangan mengaku "telat" padahal sebenarnya kesiangan, pengajuan sub-jenis **Telat oleh karyawan sendiri** untuk suatu tanggal **hanya bisa dikirim sebelum jam 08.00 WIB pada tanggal izin itu** — lewat dari jam itu ditolak **400** (pesan menyebut tanggalnya). Sub-jenis lain (Izin di Tengah Jam Kerja, Pulang Cepat) **tidak kena** batas ini.

**Pengecualian admin/owner:** kalau yang input adalah admin/owner atas nama karyawan (Feature 20, `/admin/ajukan-izin` atau "+ Ajukan Izin" di halaman Edit Karyawan), batas jam 08.00 **tidak berlaku** — keterlambatan admin menginput bukan salah karyawan yang sudah lapor sebelum jam 8. Ditandai lewat parameter `diajukanOlehAdmin: true` yang dikirim `app/api/admin/karyawan/[id]/izin/route.ts` ke `buatIzin()`; jalur karyawan sendiri (`app/api/izin/route.ts`) tidak mengirim ini jadi tetap kena batas. Validasi & catatan hint di UI juga dihapus khusus di kedua form admin — form karyawan tetap menampilkannya.

Berlaku otomatis untuk tanggal manapun: tanggal yang sudah lewat (deadline-nya sudah pasti lewat) ikut ditolak, tanggal di masa depan selalu boleh (deadline-nya belum tiba). Toko beroperasi WIB (UTC+7 tetap, tanpa DST) — dihitung manual dari string tanggal + offset UTC eksplisit (`batasWaktuIzinTelat()`, `lewatBatasIzinTelat()` di `lib/izin.ts`) supaya **tidak bergantung timezone proses** yang menjalankannya (server Vercel default UTC, bukan WIB).

**Susulan — rename label jenis izin (16 Agu 2026):** "Izin Lain-lain" → **"Izin Selain Sakit"**, "Izin Setengah Hari" → **"Izin Tidak Full (Telat, Pulang Cepat, Pertengahan)"**. Nama internal enum (`LAINNYA`, `SETENGAH_HARI`) **tidak berubah** — cuma label tampilan di `LABEL_JENIS_IZIN` (`lib/izin.ts`). Saat rename ini ditemukan **2 dari 3 form pengajuan** (karyawan & admin "+ Ajukan Izin untuk Karyawan Ini") punya dropdown `<option>` yang di-hardcode manual (tidak lewat `LABEL_JENIS_IZIN`), beda dari form admin "Ajukan Izin" yang sudah generik — jadi rename di `lib/izin.ts` saja tidak cukup nyampe ke situ. Sekalian diperbaiki: kedua dropdown itu diubah jadi generik (`Object.entries(LABEL_JENIS_IZIN).map(...)`), supaya rename/tambah jenis izin berikutnya otomatis konsisten di ketiganya tanpa perlu diingat manual lagi. Heading kecil di atas pilihan sub-jenis ("Jenis Izin Setengah Hari") ikut diganti jadi "Detail Izin Tidak Full" di ketiga form.
...
| jamMasuk, jamKeluar, jamPulang | String "HH:mm", hanya diisi kalau `jenis` = `SETENGAH_HARI` (Feature 32). `jamMasuk` dipakai bersama TELAT (jam masuk aktual) & PERTENGAHAN (jam masuk kembali). Sub-jenis TELAT **yang diajukan karyawan sendiri** punya batas waktu pengajuan: hanya sebelum jam 08.00 WIB pada tanggal izin — **tidak berlaku kalau admin/owner yang input** (Feature 32 susulan) |
