...
- `lib/izin.ts` — konstanta MURNI, client-safe: `JENIS_IZIN`, `LABEL_JENIS_IZIN`, `JENIS_SATU_TANGGAL` (Tugas Negara & Setengah Hari — hanya 1 tanggal, UI 1 input + servernya menolak rentang), `JENIS_TIDAK_HITUNG_KEHADIRAN` (Setengah Hari — dikecualikan penuh dari persentase kehadiran), `MAKS_HARI_MENIKAH` (7), `jumlahHari()`. JANGAN tambah logika server (prisma/fs) di sini — dipakai komponen client.
...
- `lib/approval.ts` — `prosesIzin(id, status, feedback?)`, `prosesTukarLibur(id, status, feedback?)` (update status + `feedbackAdmin` opsional + notif hasil yang menyertakan feedback kalau diisi), dipakai rute admin & `app/api/admin/agent-approval`. Modal input feedback (Tolak saja, bukan Setujui): `components/ModalTolakPengajuan.tsx`. Halaman kelola SEMUA pengajuan izin (bukan cuma 10 terbaru di dashboard): `/admin/izin` + `GET /api/admin/izin`, pola sama dengan `/admin/tukar-libur`.
...
- Izin: `SAKIT` (2 tipe: tanpa/dengan surat dokter, PDF/JPG/PNG ≤ 5 MB — **opsional, tidak ada lagi validasi wajib berdasar jumlah hari**, dihapus per permintaan user), `LAINNYA`, `SETENGAH_HARI` (1 tanggal, alasan WAJIB), `MENIKAH` (maks 7 hari), `TUGAS_NEGARA` (1 tanggal, alasan opsional). `SETENGAH_HARI` & `TUGAS_NEGARA` terdaftar di `JENIS_SATU_TANGGAL` (`lib/izin.ts`) — kalau nambah jenis izin 1-tanggal baru, daftarkan di sana, jangan bikin pengecekan `jenis === "..."` baru yang terpisah. Tukar libur = model terpisah `TukarLibur`, di UI karyawan menyatu di dropdown "Jenis Pengajuan". **`SETENGAH_HARI` TIDAK dihitung sama sekali dalam persentase kehadiran** (bukan 0,5 hari — dikecualikan penuh) via `JENIS_TIDAK_HITUNG_KEHADIRAN` (`lib/izin.ts`), disaring di `where` Prisma SEBELUM masuk `hitungPersenKehadiran()` — fungsi itu sendiri tidak tahu jenis izin (lihat komentar di `lib/kehadiran.ts`). Catatan: teks resmi peraturan toko (`lib/kontrak.ts`, `ISI_KONTRAK_DEFAULT`) MASIH menyebut aturan lama ("izin sakit >1 hari wajib surat dokter") — belum diubah karena itu dokumen resmi (lihat aturan #12), perlu keputusan user terpisah kalau mau disinkronkan.
- **Cegah duplikasi izin (Feature 30):** tidak bisa ajukan jenis izin yang sama untuk tanggal yang sama/tumpang tindih dengan pengajuan yang masih MENUNGGU atau sudah DISETUJUI. Izin DITOLAK tidak menghalangi ajuan ulang. **Pengecualian:** izin Sakit tanpa surat dokter boleh dilengkapi surat dokter belakangan untuk tanggal yang sama — ini **UPDATE baris yang sudah ada**, BUKAN `create` baru (supaya jumlah izin tetap terhitung 1 hari di semua laporan, bukan 2). Kalau bikin fitur baru yang menghitung/menampilkan jumlah izin, tidak perlu dedup manual — datanya memang sudah 1 baris. Cek ada di `buatIzin()` (`lib/pengajuan-izin.ts`) — kalau nambah jalur baru untuk membuat izin (selain `/api/izin` dan `/api/admin/karyawan/[id]/izin`), WAJIB lewat `buatIzin()` juga supaya proteksi ini tidak bisa dilewati.
...
18. **Menambah kartu statistik dashboard tanpa `href`, padahal harusnya bisa diklik tembus.** Kejadian nyata: kartu "Izin Menunggu" di `app/admin/page.tsx` dibuat tanpa `href` sejak awal (beda dari "Tukar Libur Menunggu", "Kontrak Perlu Diperpanjang" yang semua punya href) — admin tidak bisa klik untuk lihat/proses izin yang menunggu, dan tabel "Pengajuan Izin Terbaru" cuma nampung 10 terbaru (bukan filter MENUNGGU), jadi izin lama yang masih menunggu bisa terkubur. → *Aturan: setiap kartu dashboard yang menunjukkan angka "menunggu/perlu tindakan" WAJIB punya `href` ke halaman yang bisa memprosesnya, bukan cuma menampilkan angka. Kalau halaman itu belum ada, buat dulu (lihat `/admin/izin` sebagai contoh) — jangan biarkan angkanya "buntu".*
